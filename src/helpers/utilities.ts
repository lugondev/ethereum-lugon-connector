...
export function valueToHex(amount: number) {
    return decimalToHex(amount * 10 ** 18);
}


export function etherValue(value: number, decimals?: number) {
    const valueString = utils.parseEther(value.toString()).toString();
    if (decimals && decimals > 0 && decimals < 18) {
        return valueString.substr(0, valueString.length - (18 - decimals));
    }
    return valueString;
}


export function humanBalance(value: number, options?: { fixed?: number, decimals?: number }) {
    return parseFloat((value / 10 ** (options?.decimals || 18)).toFixed(options?.fixed || 5));
}

export function showShortAddress(address: string) {
    let first = address.substring(0, 6);
    let last = address.substring(address.length, address.length - 4);
    return `${first}...${last}`;
}
